...
const AsciiMath = {
  ...
  toTeX ( str: string ): string {
    ...
    return new AsciiMath2TeX ().parse ( str );
...
//URL: https://github.com/christianp/asciimath2tex/blob/master/asciimath2tex.js
//TODO: Rewrite this better/replace it with the properly published NPM module
...
class AsciiMath2TeX {
  ...
  binary_symbols: any[];
  left_brackets: any[];
  leftright_brackets: any[];
  non_constant_symbols: any[];
  right_brackets: any[];
  unary_symbols: any[];
  _source: any;
  brackets: any[];
  ...
  matrix(pos = 0) {
      let left: any = this.left_bracket(pos);
      let lr = false;
      if(!left) {
  ...
  matrix_contents(pos = 0, leftright = false) {
      let rows: any[] = [];
      let end = pos;
      let row_length: any = undefined;
      let column_desc: any = undefined;
      let is_array = false;
